/* =========================================================
 * Bootstrap year calendarYear v1.1.0
 * Repo: https://github.com/Paul-DS/bootstrap-year-calendarYear
 * =========================================================
 ...
 * ========================================================= */
 ...
 (function($) {
	var calendarYear = function(element, options) {
		...
		this.element.addClass('calendarYear');
	...
	calendarYear.prototype = {
		...
		_renderHeader: function() {
			...
			header.addClass('calendarYear-header panel panel-default');
		...
		_applyEvents: function () {
			...
			this.element.find('.calendarYear-header .prev').click(function() {
				if(!$(this).hasClass('disabled')) {
			...
			this.element.find('.calendarYear-header .next').click(function() {
				if(!$(this).hasClass('disabled')) {
					_this.element.find('.months-container').animate({'margin-left':'-100%'},100, function() {
						_this.element.find('.months-container').css('visibility', 'hidden');
						_this.element.find('.months-container').css('margin-left', '0');
			...
			setInterval(function() {
				var calendarYearSize = $(_this.element).width();
				var monthSize = $(_this.element).find('.month').first().width() + 10;
				var monthContainerClass = 'month-container';

				if(monthSize * 6 < calendarYearSize) {
					monthContainerClass += ' col-xs-2';
				}
				else if(monthSize * 4 < calendarYearSize) {
					monthContainerClass += ' col-xs-3';
				}
				else if(monthSize * 3 < calendarYearSize) {
					monthContainerClass += ' col-xs-4';
				}
				else if(monthSize * 2 < calendarYearSize) {
		...
		_openContextMenu: function(elt) {
			var contextMenu = $('.calendarYear-context-menu');
			...
			if(contextMenu.length > 0) {
				...
				contextMenu.addClass('calendarYear-context-menu');
	...
	$.fn.calendarYear = function (options) {
		var calendarYear = new calendarYear($(this) ,options);
		$(this).data('calendarYear', calendarYear);
		return calendarYear;
	}
	...
	var dates = $.fn.calendarYear.dates = {
		en: {
	...
	var colors = $.fn.calendarYear.colors = ['#2C8FC9', '#9CB703', '#F5BB00', '#FF4A32', '#B56CE2', '#45A597'];

	$(function(){
		$('[data-provide="calendarYear"]').each(function() {
			$(this).calendarYear();
		});
	});
 }(window.jQuery));